# WebAR QRコード画像/パンフレットプロジェクト

スマートフォンで QR コードを読み取り、QRコードが書かれた画像（またはパンフレット等の印刷物）をカメラに映すと 3D キャラクターが表示される WebAR アプリケーションです。
...
│   └── targets/
│       └── card.mind       # 画像ターゲット（QRコード画像/パンフレットの図柄から生成）
...
- `./assets/targets/card.mind` - 画像ターゲット（QRコード画像/パンフレットの図柄から生成）
...
**症状**: ターゲット画像（QRコード画像/パンフレット等）を映しても 3D モデルが表示されない
